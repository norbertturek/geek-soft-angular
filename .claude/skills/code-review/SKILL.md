...
Apply the project's conventions from `AGENTS.md` / `.cursor/rules/cursor.mdc`: standalone components, signals, `input()`/`output()`, OnPush, native control flow, Reactive forms, accessibility (AXE, WCAG AA), color palette (`var(--color-*)`), etc.
...
- [ ] **Colors**: Use `var(--color-*)` only; no hardcoded palette colors
